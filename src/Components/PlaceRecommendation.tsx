...
function PlaceRecommendation(place_id) {
    ...
    const currentPlace = places.filter(place => place.place_id === place_id.place_id)
    const hygieneResult = useHygieneCheck(currentPlace[0].RatingValue);

        
    return ( 
        <Card className="bg-[#F3F3F3] border-none  shrink-0 ">
            <div className="flex p-2 gap-x-2">
                <div className="flex relative w-[150px] xs:w-[200px] h-full">
                    {  <PlaceImage images={currentPlace[0].photos} className="rounded-lg " />}
